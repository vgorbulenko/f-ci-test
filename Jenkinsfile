...
env.bucketName = "s3://frustum-temp/installer"
	...
	node('master') {
		ws( env.wsPath ) {
			stage('Versioning') {
				...
				bat """
					@echo off
					echo ==== Modifying Resources.resx ====
					cd %WORKSPACE%\\Generate.WebInstaller\\Generate.WebInstaller\\Properties
					powershell -Command "(gc Resources.resx) -replace 'generate-app.frustum.com/installer', 'generate-app.frustum.com/installer/%GenerateBuildStage%' | Out-File -encoding UTF8 Resources.resx"
				"""
			}
		}
	}
		...
		ws( env.wsPath ) {
			...
			stage('Returning results') {
				...
				bat """
					...
					echo ==== Genetare version.json ====
					::echo %GenerateBuildVersion% version.json > %dir_installers%\\version.json
					echo { > %dir_installers%\\version.json
					echo   \"version\": \"%GenerateBuildVersion%\" >> %dir_installers%\\version.json
					echo } >> %dir_installers%\\version.json
					"""
				bat """
					@echo off
